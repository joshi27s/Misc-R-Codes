...
```{r data setup}
#Load the data
setwd("C:/PhD Spring quarter/582/HW4")
loadPath = getwd()
```
```{r}
...
stat_function(fun = dnorm, args = (list(mean = cbeta, sd = sqrt(Vhat.irrelevant/n.obs)))) +
...
ggplot(sim.weak) +
geom_histogram(aes(x=beta_hat, y=..density..), bins = 30, alpha = 0.3) +
stat_function(fun = dnorm, args = (list(mean = cbeta, sd = sqrt(Vhat.weak/n.obs)))) +
...
ggplot(sim.strong) +
geom_histogram(aes(x=beta_hat, y=..density..), bins = 30, alpha = 0.3) +
stat_function(fun = dnorm, args = (list(mean = cbeta, sd = sqrt(Vhat.strong/n.obs)))) +
